...
pub struct DesktopCtx {
    indicate_canal: mpsc::Sender<String>,
    prompt_canal: RefCell<(mpsc::Sender<String>, mpsc::Receiver<String>)>,
}
...
impl SyncContext for DesktopCtx {
    type Instance = DesktopInstance;
    type FileRead = File;
...
impl InitInstanceContext for DesktopCtx {
    type CharsDist = DesktopCharDist;
    ...
    fn key_rng(&self) -> impl CryptoRngCore + 'static {
        OsRng
    }
    fn salt_rng(&self) -> impl CryptoRngCore + 'static {
        OsRng
    }
}

impl Instance<DesktopCtx> for DesktopInstance {
    fn open(loc: Option<<DesktopCtx as Context>::InstanceLoc>) -> Result<Self, errors::Error> {
    ...
    fn location(&self) -> <DesktopCtx as Context>::InstanceLoc {
        if self.is_global {
            RepoPath::Global
        } else {
    ...
    fn get_element<const IS_PASSWORD: bool>(
        &self,
        path: &<DesktopCtx as Context>::FileLeaf<IS_PASSWORD>,
    ...
    fn list_content<const IS_PASSWORD: bool>(
        &self,
        directory: <DesktopCtx as Context>::FilePath<IS_PASSWORD>,
    ) -> Result<
        impl Iterator<
            Item = Result<
                yoursb_domain::interfaces::PathOrLeaf<DesktopCtx, IS_PASSWORD>,
                errors::Error,
            >,
        >,
        errors::Error,
...
impl WritableInstance<DesktopCtx> for DesktopInstance {
    ...
    fn delete(self) -> std::result::Result<(), (errors::Error, DesktopInstance)> {
...
///
/// It can be parsed from a string, see the [`allowed_chars`](crate::Desktop.allowed_chars)
/// command-line argument for more details.
#[derive(Clone, Debug)]
pub struct DesktopCharDist(Vec<(char, char)>);

/// An error kind indicating the parsing of a [`DesktopCharDist`] didn't go well.
#[derive(Clone, Debug)]
...
impl FromStr for DesktopCharDist {
...
impl CharsDist for DesktopCharDist {
